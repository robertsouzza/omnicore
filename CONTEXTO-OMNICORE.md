...
> **Como usar em chat novo:** digite `@CONTEXTO-OMNICORE.md` ou peça: *"Leia CONTEXTO-OMNICORE.md e continue de onde paramos"*.
>
...
└── CONTEXTO-OMNICORE.md ← este arquivo
```

### Workspace Cursor (importante)

- Chat do Agent fica ligado ao **workspace**, não à pasta git.
- Conversa completa existe em workspace `cerebro-backend`.
- Abrir `omnicore.code-workspace` ou `~/omnicore` = **chat novo/vazio**.
- **Recomendação:** manter workspace em `cerebro-backend` (+ Add Folder `frontend-app` se quiser) para não perder histórico.
...
```
Olá Logan, leia @CONTEXTO-OMNICORE.md e vamos continuar o OmniCore.
Próximo: Opção A — setup React em frontend-app/ com login JWT.
Workspace: cerebro-backend (+ frontend-app). Não commitar docker-compose.yml.
```

---

*Última atualização: 14/ago/2026 — auth JWT validado no Swagger; git em `434b7f3` pushed.*
